...
class Graph {
    ...
    bool DFS_for_Cyclic(int v, bool visited[],  queue<int>& Q,bool *stack,int& berker);
public:
    Graph(int vertices);
...
bool Graph::DFS_for_Cyclic(int v, bool visited[],queue<int>& Q,bool *stack,int& berker )
{
    if(visited[v] == false)
    {
       int p=-1;
       int r=-1;
        ...
        {
            if ( !visited[*i] && DFS_for_Cyclic(*i, visited, Q,stack,berker) ){
                r=*i;
                if(v==Q.front()){
                    Q.push(r);
                    Q.push(v);
...
bool Graph::isCyclic(std::vector<int> &ans)
{
    ...
    for(int i = 0; i < vertices; i++){
        clear(Q);
        if (DFS_for_Cyclic(i, visited, Q,stack, berker) )
            flag=1;
        if (flag==1)
            break;
            }
